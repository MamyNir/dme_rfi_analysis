...
def setup_logging(file, level: str = "INFO") -> None:
    ...
    script_dirr = os.path.dirname(os.path.abspath(__file__))
    log_dir = os.path.join(script_dirr, "logs")
...
def process_file(args):
    ...
    try:
        ...
        dirr_name = f"prob_{initial}_{int(freq_min)}_{int(freq_max)}"
        root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        dirr_result = os.path.join(root, "results", "lband", dirr_name, file_name)
